...
export default function Security({
    ...
    const updatePassword = (e: React.FormEvent) => {
        e.preventDefault();
        passwordForm.post(SecurityController.update.url(), {
            preserveScroll: true,
                ...
                >
                    <div className="mb-12 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                        <div className="flex items-center gap-6">
                            ...
                            <div>
                                <h2 className="text-2xl font-black tracking-tight text-slate-900 dark:text-foreground uppercase italic">{__('Security Protocol')}</h2>
                                <p className="text-slate-500 dark:text-muted-foreground/60 font-medium">{__('Fortify your presence with a complex digital cipher.')}</p>
                            </div>
                        </div>
                    ...
                    >
                        <>
                                <div className="sm:col-span-2 grid gap-3">
                                    <Label htmlFor="current_password" className="ml-2 text-[10px] font-black uppercase tracking-[0.3em] text-sky-600 dark:text-sky-500/70">
                                     ...
                                     <div className="group relative">
                                        <Lock size={20} className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-300 dark:text-muted-foreground/30 group-focus-within:text-sky-600 dark:group-focus-within:text-sky-500 transition-all duration-500" />
                                        <PasswordInput
                                            id="current_password"
                                    ...
                                    <InputError className="ml-2" message={passwordForm.errors.current_password} />
                                </div>
                                ...
                                <div className="grid gap-3">
                                    ...
                                    <InputError className="ml-2" message={passwordForm.errors.password} />
                                </div>
                                ...
                                <div className="grid gap-3">
                                    ...
                                    <InputError className="ml-2" message={passwordForm.errors.password_confirmation} />
                                </div>
                                ...
                                <div className="sm:col-span-2 flex flex-col sm:flex-row items-center gap-8 pt-6">
                                    ...
                                    >
                                        {passwordForm.processing ? __('Processing...') : __('Sync New Cipher')}
                                    </Button>
                    ...
                    >
                        <div className="mb-12 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                            <div className="flex items-center gap-6">
                                ...
                                <div>
                                    <h2 className="text-2xl font-black tracking-tight text-slate-900 dark:text-foreground uppercase italic">{__('Multi-Factor Gateway')}</h2>
                                    <p className="text-slate-500 dark:text-muted-foreground/60 font-medium">{__('Add an impenetrable layer to your digital sanctuary.')}</p>
                                </div>
                            </div>
                            {twoFactorEnabled && (
                                <motion.div 
                         ...
                         {twoFactorEnabled ? (
                            <div className="space-y-10">
                                ...
                                <div className="flex items-center gap-6">
                                    <form onSubmit={(e) => {
                                        e.preventDefault();
                                        disable2faForm.post(disable.url());
                                    }}>
                                        <Button
                                            variant="destructive"
                                            type="submit"
                            ...
                            <div className="space-y-10">
                                <div className="rounded-[2rem] border border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-white/5 p-8 flex items-start gap-6">
                                    <div className="h-12 w-12 rounded-2xl bg-slate-100 dark:bg-white/5 flex items-center justify-center text-slate-400 dark:text-muted-foreground shrink-0">
                                        <ShieldAlert size={24} />
                                    </div>
                                    <p className="text-slate-500 dark:text-muted-foreground font-medium leading-relaxed italic">
