...
import { CalendarRange, Clock, Users, ArrowLeft, Trash2, Utensils, Plus, Minus, PencilLine, MessageCircle, QrCode, ShieldCheck } from 'lucide-react';
...
import { history as bookingHistory } from '@/routes/bookings';
...
export default function ReservationShow({ auth, reservation, availableMenus }: any) {
    ...
    return (
        <div className="min-h-screen bg-background font-sans text-foreground selection:bg-amber-100 selection:text-amber-900 transition-colors duration-500">
            ...
            <Navbar auth={auth} dashboardUrl={dashboardUrl} mobileMenuOpen={mobileMenuOpen} setMobileMenuOpen={setMobileMenuOpen} />

            <main className="pt-24 pb-20 sm:pt-32 lg:pb-28">
                <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
                    
                    <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
                        <Link href={bookingHistory().url} className="inline-flex items-center gap-2 text-sm font-medium text-muted-foreground hover:text-amber-600 transition-colors">
                        ...
                        {isEditing && (
                            <div className="flex gap-3">
                                <Button onClick={() => {
 setIsEditing(false); setData('menus', reservation.menus) 
