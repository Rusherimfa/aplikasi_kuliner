    ...
    useEffect(() => {
        ...
        const resChannel = window.Echo.channel(`reservations.${reservation.id}`)
            .listen('.ReservationStatusUpdated', (e: any) => {
                router.reload({ 
                    preserveScroll: true,
                    onSuccess: () => toast.success('Status reservasi Anda telah diperbarui!')
                });
            })
            .listen('.DishStatusUpdated', (e: any) => {
                router.reload({ 
                    preserveScroll: true,
                    onSuccess: () => toast.success(`Piring "${e.itemName}" kini ${e.status.toUpperCase()}`)
                });
            })
                        ...
                        <div className="flex gap-4">
                            {isConfirmed && reservation.status !== 'completed' && !isEditing && (
                                <Button onClick={() => router.put(`/reservations/${reservation.id}`, { status: 'completed' })} className="h-11 px-6 rounded-2xl bg-emerald-500 text-black font-black uppercase tracking-widest hover:bg-white transition-all">
                                    <CheckCircle2 size={14} className="mr-2" /> Selesaikan Kedatangan
                                </Button>
                            )}
                            {isPending && !isEditing && (
                        ...
                        <div className="space-y-10">
                            {reservation.checked_in_at && reservation.status === 'confirmed' && (
                                <motion.div
                                    ...
                                    className="bg-orange-500 rounded-[2.5rem] p-8 shadow-2xl shadow-orange-500/20 text-black relative overflow-hidden"
                                >
                                    ...
                                    <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
                                        <div className="text-center md:text-left">
                                            ...
                                            <h2 className="text-2xl font-black font-['Playfair_Display',serif] tracking-tight leading-tight">Butuh <span className="italic font-serif opacity-40">Bantuan?</span></h2>
                                            <p className="text-[9px] font-bold uppercase tracking-widest mt-1 opacity-60">Satu sentuhan untuk pelayanan eksklusif kami.</p>
                                        </div>

                                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 w-full md:w-auto">
                                            {[
                                                { type: 'waiter', label: 'Pelayan', icon: Bell },
                                                { type: 'bill', label: 'Minta Bill', icon: CreditCard },
                                                { type: 'refill', label: 'Refill Air', icon: Utensils },
                                                { type: 'napkins', label: 'Tissue', icon: Sparkles },
                                            ].map((service) => (
                                                <button
                                                    key={service.type}
                                                    onClick={() => {
                                                        http.post('/service-requests', {
                                                            ...
                                                            onSuccess: () => {
                                                                toast.success('Permintaan Terkirim', {
                                                                    description: 'Staff kami akan segera menuju meja Anda.'
                                                                });
                                                            },
                                                            onError: () => {
                                ...
                                <div className="p-10 md:p-14">
                                    ...
                                    <div className="mt-12 mb-8">
                                        <div className="flex items-center justify-between relative">
                                            ...
                                            {[
                                                { label: 'Booked', icon: Ticket, active: true },
                                                { label: 'Check-in', icon: ShieldCheck, active: isCheckedIn },
                                                { label: 'Cooking', icon: ChefHat, active: reservation.menus.some((m: any) => m.pivot?.status === 'cooking' || m.pivot?.status === 'preparing') },
                                                { label: 'Enjoy', icon: Utensils, active: reservation.status === 'completed' || reservation.menus.every((m: any) => m.pivot?.status === 'ready' || m.pivot?.status === 'served') }
                                            ].map((step, idx) => (
                                                <div key={idx} className="relative z-10 flex flex-col items-center">
                                                    <div className={`h-10 w-10 rounded-full flex items-center justify-center border-2 transition-all duration-700 ${step.active ? 'bg-orange-500 border-orange-500 text-[#0A0A0B]' : 'bg-slate-50 dark:bg-[#0A0A0B] border-slate-200 dark:border-white/10 text-slate-300 dark:text-white/20'}`}>
                                                        <step.icon size={18} />
                                                    </div>
                                                    <span className={`text-[9px] font-black uppercase tracking-widest mt-2 ${step.active ? 'text-orange-500' : 'text-slate-300 dark:text-white/10'}`}>{step.label}</span>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                            ...
                            {reservation.review && (
                                <motion.div 
                                    initial={{ opacity: 0, y: 20 }}
                                    animate={{ opacity: 1, y: 0 }}
                                    className="rounded-[3rem] border border-orange-500/20 bg-orange-500/5 p-10 shadow-3xl text-center"
                                >
                                    <div className="h-16 w-16 rounded-full bg-orange-500 mx-auto mb-6 flex items-center justify-center text-black">
                                        <Star size={32} fill="currentColor" />
                                    </div>
                                    <h3 className="text-2xl font-black text-white font-['Playfair_Display',serif] mb-2">Terima Kasih!</h3>
                                    <p className="text-sm text-orange-500/60 font-medium">Ulasan Anda telah kami terima untuk meningkatkan pelayanan kami.</p>
                                </motion.div>
                            )}
                        </div>
                                                        ...
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            {item.pivot?.status && (
                                                                <Badge variant="outline" className={`text-[8px] font-black uppercase tracking-widest px-1.5 py-0 border-0 ${
                                                                    ...
                                                                    item.pivot.status === 'cooking' ? 'text-orange-400 animate-pulse' : 
...
function ReviewForm({ reservationId }: { reservationId: number }) {
    ...
    return (
        <motion.div 
            ...
            animate={{ opacity: 1, y: 0 }}
            className="rounded-[3.5rem] bg-white dark:bg-white/[0.03] border border-orange-500/30 p-12 shadow-[0_50px_100px_-20px_rgba(255,165,0,0.1)] relative overflow-hidden"
        >
            <div className="absolute top-0 right-0 p-8 opacity-10">
                <Sparkles size={120} className="text-orange-500" />
            </div>
            
            <h3 className="text-3xl font-black text-white font-['Playfair_Display',serif] tracking-tight mb-2">How was your stay?</h3>
            <p className="text-slate-400 text-sm mb-10 font-medium tracking-wide">Help us perfect the boutique experience.</p>
            
            <form onSubmit={submit} className="space-y-8 relative z-10">
                <div className="space-y-4">
                    <Label className="text-[10px] font-black uppercase tracking-[0.3em] text-orange-500/60">Boutique Rating</Label>
                    <div className="flex gap-3">
                        {[1, 2, 3, 4, 5].map((star) => (
                            <button
                                key={star}
                                ...
                                className={`h-12 w-12 rounded-xl flex items-center justify-center transition-all ${data.rating >= star ? 'bg-orange-500 text-black shadow-lg shadow-orange-500/20 scale-110' : 'bg-white/5 text-white/20 hover:bg-white/10'}`}
                            >
                ...
                <div className="space-y-4">
                    <Label className="text-[10px] font-black uppercase tracking-[0.3em] text-orange-500/60">Share Your Experience</Label>
                    <Textarea 
                        placeholder="What did you love about your gastronomy journey?"
                        value={data.message}
                        onChange={e => setData('message', e.target.value)}
                        className="rounded-[2rem] bg-black/40 border-white/5 focus:border-orange-500/50 min-h-[140px] p-6 text-white"
                    />
                </div>

                <Button disabled={processing} className="w-full h-16 rounded-[2rem] bg-orange-500 text-black font-black uppercase tracking-[0.2em] hover:bg-white transition-all shadow-xl shadow-orange-500/20">
                    {processing ? 'Sharing Experience...' : 'Post Testimonial'}
                </Button>
            </form>
        </motion.div>
    );
