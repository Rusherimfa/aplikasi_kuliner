...
export default function ReservationHistory({ auth, reservations }: any) {
    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-neutral-950/50 font-sans text-slate-900 selection:bg-amber-100 selection:text-amber-900 dark:text-white dark:selection:bg-amber-900/30 dark:selection:text-amber-300">
            ...
            <main className="pt-24 pb-20 sm:pt-32 lg:pb-28">
                <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="mb-10 md:mb-16">
                        <h1 className="font-['Playfair_Display',serif] text-4xl font-bold text-slate-900 dark:text-white md:text-5xl mb-4">
                    ...
                    {reservations.length === 0 ? (
                        <div className="flex flex-col items-center justify-center rounded-[2rem] border border-dashed border-slate-300 bg-white/50 px-6 py-20 text-center dark:border-neutral-800 dark:bg-neutral-900/50">
                            ...
                            <h3 className="mb-3 text-xl font-bold text-slate-900 dark:text-white">Belum Ada Reservasi</h3>
                            <p className="mb-8 max-w-sm text-slate-500 dark:text-neutral-400 leading-relaxed">
                        ...
                        <div className="grid gap-6">
                            {reservations.map((r: any) => (
                                <Link href={`/reservations/${r.id}`} key={r.id} className="group flex flex-col gap-5 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition-all hover:shadow-md hover:border-amber-200 dark:border-neutral-800 dark:bg-neutral-900 dark:hover:border-neutral-700 sm:flex-row sm:items-center sm:p-8">
                                    <div className="flex-1">
                                        ...
                                        <div className="flex flex-wrap gap-4 text-sm text-slate-500 dark:text-neutral-400">
                                        ...
                                        {r.special_requests && (
                                            <div className="mt-4 rounded-xl bg-slate-50 p-4 text-sm text-slate-600 dark:bg-neutral-950 dark:text-neutral-400">
                                                <span className="block font-semibold mb-1 text-slate-700 dark:text-neutral-300">Catatan Khusus:</span>
                                                "{r.special_requests}"
                                            </div>
                                        )}
                                    </div>
                                    <div className="shrink-0 pt-4 sm:pt-0 sm:border-l sm:border-slate-100 sm:pl-8 dark:sm:border-neutral-800">
                                        <p className="text-xs font-semibold tracking-wider text-slate-400 dark:text-neutral-500 uppercase mb-2">ID Reservasi</p>
                                        <p className="font-mono text-sm text-slate-700 dark:text-neutral-300">#RES-{r.id.toString().padStart(4, '0')}</p>
