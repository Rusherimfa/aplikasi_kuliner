                            ...
                            <form onSubmit={handleSubmit} className="space-y-12">
                                ...
                                >
                                    <h2 className="text-2xl font-black text-slate-900 dark:text-white font-['Playfair_Display',serif] tracking-tight mb-10 flex items-center gap-4">
                                        <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-sky-500/10 text-sky-500 border border-sky-500/20 font-black text-sm">01</div>
                                        Identity
                                    </h2>
                                    <div className="space-y-8">
                                        <div className="space-y-3">
                                            ...
                                            <Input required value={formData.customer_name} onChange={(e) => setFormData({...formData, customer_name: e.target.value})} placeholder="Masukkan nama Anda" className="h-16 rounded-2xl bg-slate-50 dark:bg-black/20 border-slate-200 dark:border-white/5 text-lg font-bold px-6 focus:ring-sky-500/20" />
                                        </div>
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                                            <div className="space-y-3">
                                                ...
                                                <Input required value={formData.customer_email} onChange={(e) => setFormData({...formData, customer_email: e.target.value})} type="email" placeholder="contoh@email.com" className="h-16 rounded-2xl bg-slate-50 dark:bg-black/20 border-slate-200 dark:border-white/5 text-lg font-bold px-6 focus:ring-sky-500/20" />
                                            </div>
                                            <div className="space-y-3">
                                        ...
                                        {orderType === 'delivery' && (
                                            <div className="pt-4">
                                                <Button 
                                                    ...
                                                    variant="outline"
                                                    className={`w-full h-16 rounded-2xl border-2 flex items-center justify-center gap-4 transition-all duration-500 ${
                                                        locationStatus === 'success' 
                                                            ? 'border-emerald-500 bg-emerald-500/10 text-emerald-500' 
                                                            : 'border-slate-200 dark:border-white/10 hover:border-sky-500/50'
                                                    }`}
                                                >
                                                    <div className={`h-8 w-8 rounded-lg flex items-center justify-center ${locationStatus === 'success' ? 'bg-emerald-500 text-white' : 'bg-sky-500/10 text-sky-500'}`}>
                                                        <MapPin size={18} className={locationStatus === 'loading' ? 'animate-bounce' : ''} />
                                                    </div>
                                                    <div className="text-left">
                                                        <p className="text-[10px] font-black uppercase tracking-widest">
                                                            {locationStatus === 'loading' ? 'Locating...' : locationStatus === 'success' ? 'Location Locked' : 'Precision Delivery Point'}
                                                        </p>
                                                        <p className="text-[9px] font-medium opacity-60">
                                                            {locationStatus === 'success' ? 'Titik koordinat berhasil dikunci.' : 'Klik untuk mendapatkan koordinat GPS Anda secara otomatis.'}
                                                        </p>
                                                    </div>
                                                </Button>
                                ...
                                >
                                    <h2 className="text-2xl font-black text-slate-900 dark:text-white font-['Playfair_Display',serif] tracking-tight mb-10 flex items-center gap-4">
                                        <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-sky-500/10 text-sky-500 border border-sky-500/20 font-black text-sm">02</div>
                                        Experience Mode
                                    </h2>
                                     <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        ...
                                        >
                                            ...
                                            <div className="flex items-center justify-between mb-4">
                                                ...
                                                {orderType === 'delivery' && <CheckCircle2 size={24} className="text-sky-500" />}
                                            </div>
                                            <div className={`font-black uppercase tracking-widest text-xs ${orderType === 'delivery' ? 'text-sky-500' : 'text-slate-400'}`}>Delivery</div>
                                        ...
                                        >
                                            ...
                                            <div className="flex items-center justify-between mb-4">
                                                ...
                                                {orderType === 'pickup' && <CheckCircle2 size={24} className="text-sky-500" />}
                                            </div>
                                            <div className={`font-black uppercase tracking-widest text-xs ${orderType === 'pickup' ? 'text-sky-500' : 'text-slate-400'}`}>Ambil Di Tempat</div>
                            ...
                            >
                                <div className="flex items-center justify-between mb-10">
                                    <h3 className="text-2xl font-black font-['Playfair_Display',serif] text-slate-900 dark:text-white tracking-tight">
                                        Seafood <span className="italic opacity-40 text-sky-500 text-3xl block">Selection</span>
                                    </h3>
                                    <div className="h-12 w-12 rounded-2xl bg-sky-500/10 flex items-center justify-center text-sky-500 border border-sky-500/20 font-black text-xs">
                                        {items.length}
                                    </div>
                                </div>
                                ...
                                {items.length > 0 ? (
                                    ...
                                    <div className="py-20 text-center rounded-[2.5rem] border border-dashed border-white/5 bg-white/[0.01] mb-10">
                                        ...
                                        <Link href="/catalog" className="inline-flex items-center gap-2 text-sky-500 text-[10px] font-black uppercase tracking-widest mt-6 hover:text-white transition-colors">
