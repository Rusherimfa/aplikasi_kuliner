...
export default function OrderHistory({ auth, orders }: any) {
    ...
    const getStatusBadge = (status: string) => {
        const variants: Record<string, { label: string, color: string, icon: any }> = {
            'pending': { label: 'Menunggu Konfirmasi', color: 'bg-orange-500/10 text-orange-400 border-orange-500/20', icon: Clock3 },
    ...
    return (
        <div className="min-h-screen bg-[#FAFAFA] dark:bg-[#0A0A0B] font-sans text-foreground transition-colors duration-500 selection:bg-orange-500/30 overflow-hidden">
            <Head title="Check Your Order — RestoWeb" />
                    ...
                    <div className="mb-16 flex flex-col md:flex-row md:items-end justify-between gap-8">
                        <div>
                            <div className="flex items-center gap-2 mb-4">
                                ...
                                <span className="text-[10px] font-black tracking-[0.3em] text-orange-500 uppercase">Food Tracker</span>
                            </div>
                            <h1 className="font-['Playfair_Display',serif] text-5xl font-black text-slate-900 dark:text-white md:text-6xl tracking-tighter">
                                My <span className="italic font-serif opacity-40 text-orange-500">Orders</span>
                            </h1>
                            ...
                            {orders.length === 0 ? (
                                <div className="flex flex-col items-center justify-center py-32 glass-card rounded-[3rem] border-dashed border-2">
                                    ...
                                    <p className="text-slate-500 dark:text-neutral-500 font-medium mb-12 max-w-sm text-center">
                                        Menu-menu estetik kami sudah menanti. Yuk eksplorasi Gastronomy Suite!
                                    </p>
                                    <Link href="/catalog">
                                        ...
                                        <div className="glass-card flex flex-col md:flex-row gap-8 p-8 md:p-12 rounded-[3.5rem] bg-white dark:bg-white/[0.02] border border-slate-200 dark:border-white/5 backdrop-blur-3xl shadow-3xl hover:border-orange-500/30 transition-all duration-700">
                                            ...
                                            <div className="flex-1 space-y-8">
                                                ...
                                                <div className="space-y-3">
                                                    ...
                                                    <div className="flex flex-wrap gap-6 text-sm font-bold text-slate-500 dark:text-neutral-400">
                                                        <span className="flex items-center gap-2.5"><Receipt size={16} className="text-orange-500" /> {o.order_type === 'delivery' ? 'Delivery' : 'Ambil di Tempat'}</span>
                                                        <span className="flex items-center gap-2.5"><Clock size={16} className="text-orange-500" /> {new Date(o.created_at).toLocaleString('id-ID', { dateStyle: 'medium', timeStyle: 'short' })}</span>
                                                    </div>
                                                </div>
                                                    ...
                                                    <div className="space-y-4">
                                                        ...
                                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                                            {o.items.map((item: any) => (
                                                                <div key={item.id} className="flex items-center justify-between px-4 py-3 rounded-2xl bg-orange-500/5 dark:bg-white/5 border border-orange-500/10 dark:border-white/10">
                                            ...
                                            <div className="md:w-72 flex flex-col justify-center gap-4 md:pl-12 md:border-l border-slate-200 dark:border-white/5">
                                                ...
                                                {o.order_status === 'preparing' && (
                                                    <div className="w-full h-14 bg-orange-500/10 text-orange-500 border border-orange-500/20 rounded-2xl flex items-center justify-center text-[10px] font-black uppercase tracking-widest">
                        ...
                        <div className="flex flex-col h-[80vh] md:h-auto">
                            <div className="p-8 md:p-10 !pb-6 border-b border-slate-100 dark:border-white/5 relative bg-slate-50/50 dark:bg-zinc-950/50">
                                <DialogTitle className="font-['Playfair_Display',serif] text-3xl md:text-4xl font-black text-slate-900 dark:text-white tracking-tighter">
                                    Detail <span className="italic font-serif opacity-40 text-orange-500">Pesanan</span>
                                </DialogTitle>
                            ...
                            <div className="flex-1 overflow-y-auto p-8 md:p-10 space-y-8">
                                <div className="grid grid-cols-2 gap-6 bg-slate-50 dark:bg-white/5 p-6 rounded-3xl border border-slate-100 dark:border-white/5">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm font-bold text-slate-900 dark:text-white flex items-center gap-2">
                                            {selectedOrder.order_type === 'delivery' ? <><Truck size={14} className="text-orange-500" /> Delivery</> : <><Store size={14} className="text-orange-500"/> Ambil di Tempat</>}
                                        </p>
                                    </div>
                                    <div>
                                ...
                                <div className="border-t border-slate-200 dark:border-white/10 pt-6">
                                    <div className="flex items-end justify-between bg-orange-500/5 px-6 py-5 rounded-3xl border border-orange-500/20">
                                        <div>
                                            <p className="text-[10px] font-black text-orange-500/60 uppercase tracking-[0.3em] mb-1">Payment Total</p>
