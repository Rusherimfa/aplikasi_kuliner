...
export default function StaffDashboard({ auth, stats, todays_schedule, recent_activity, revenue_chart, best_sellers, reservation_chart }: any) {
        ...
        <>
            ...
            <div className="mx-auto max-w-7xl font-sans text-white pb-20">
                ...
                <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4 mb-12">
                    {[
                        { title: __('Reservasi Menunggu'), val: stats.pending_reservations, icon: Clock, color: 'text-sky-500', bg: 'bg-sky-500/10' },
                        { title: __('Pesanan Aktif'), val: stats.active_orders, icon: CalendarCheck, color: 'text-emerald-500', bg: 'bg-emerald-500/10' },
                        { title: __('Item Terjual'), val: stats.total_food_sold, icon: HandPlatter, color: 'text-purple-400', bg: 'bg-purple-400/10' },
                                ...
                                {best_sellers.map((menu: any, idx: number) => (
                                    <div key={menu.id} className="group relative text-left">
                                        <div className="flex justify-between items-center mb-3 px-1">
                                            ...
                                            <span className="text-sm font-black text-emerald-500 italic">{menu.sold} <span className="text-[10px] not-italic opacity-40">{__('SOLD')}</span></span>
                                        </div>
                ...
                <div className="mb-12">
                    ...
                    <div className="rounded-[3rem] border border-white/5 bg-white/[0.02] shadow-3xl backdrop-blur-3xl flex flex-col ring-1 ring-white/5">
                        <div className="p-10 border-b border-white/5 flex justify-between items-center text-left">
                            <div>
                                <h2 className="text-2xl font-black text-white font-['Playfair_Display',serif] tracking-tight">{__('Jadwal Hari Ini')}</h2>
                                <p className="text-[10px] font-bold text-white/20 uppercase tracking-[0.2em] mt-1">{__('Timeline kedatangan tamu')}</p>
                            </div>
                                ...
                                <table className="w-full text-left border-collapse">
                                    <thead>
                                        <tr className="bg-white/[0.01]">
                                            <th className="p-6 text-[10px] font-black text-white/20 uppercase tracking-widest pl-10">{__('Waktu')}</th>
                                            <th className="p-6 text-[10px] font-black text-white/20 uppercase tracking-widest">{__('Nama Tamu')}</th>
                                            <th className="p-6 text-[10px] font-black text-white/20 uppercase tracking-widest text-center">{__('Meja')}</th>
                                            <th className="p-6 text-[10px] font-black text-white/20 uppercase tracking-widest text-right pr-10">{__('Status')}</th>
                                        </tr>
                                    </thead>
                                        ...
                                        {todays_schedule.map((res: any) => (
                                            <tr key={res.id} className="hover:bg-white/[0.03] transition-colors group">
                                                ...
                                                <td className="p-6">
                                                    ...
                                                    <p className="text-[10px] text-white/20 font-bold mt-1 tracking-widest">{res.guests} {__('GUESTS')}</p>
                                                </td>
                                                    ...
                                                    <div className="flex items-center justify-end gap-3">
                                                        ...
                                                        <button 
                                                            ...
                                                            className={`p-1.5 rounded-lg border transition-colors ${
                                                                activeChatId === res.id ? 'bg-emerald-500 text-black border-emerald-500' : 'bg-white/5 text-white/40 hover:bg-white/10 hover:text-white border-white/5'
                                                            }`}
                                                            title={__('Buka Chat')}
                                                        >
