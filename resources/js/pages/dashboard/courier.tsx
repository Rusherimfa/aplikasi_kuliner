            ...
            <div className="mx-auto max-w-7xl font-sans text-white pb-20">
                ...
                <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-2 mb-12">
                    {[
                        ...
                        { title: 'Selesai Hari Ini', val: stats.completed_deliveries, icon: CheckCircle2, color: 'text-emerald-400', bg: 'bg-emerald-400/10' },
                    ].map((card, idx) => (
                        <div 
                            key={idx} 
                                    ...
                                    <div key={delivery.id} className="p-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 hover:bg-white/[0.01] transition-colors">
                                        ...
                                        <div className="flex items-center gap-3 w-full md:w-auto">
                                            {delivery.status === 'preparing' && (
                                                <Button 
                                                    onClick={() => router.patch(`/reservations/${delivery.id}/delivery-status`, { delivery_status: 'on_delivery' })}
                                             ...
                                             {delivery.status === 'on_delivery' && (
                                                 <div className="flex flex-col gap-2 w-full md:w-auto">
                                                     <Button 
                                                         onClick={() => {
                                                             http.post(`/reservations/${delivery.id}/simulate-tracking`);
                                                             ...
                                                             const interval = setInterval(() => {
                                                                 ...
                                                                 http.post(`/reservations/${delivery.id}/simulate-tracking`, { latitude: lat, longitude: lng });
                                                                 i++;
                                                             }, 2000);
                                                         }}
                                                         variant="outline"
                                                     ...
                                                     <Button 
                                                         onClick={() => router.patch(`/reservations/${delivery.id}/delivery-status`, { delivery_status: 'delivered' })}
