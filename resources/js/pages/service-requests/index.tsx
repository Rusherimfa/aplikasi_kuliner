...
export default function ServiceRequestsIndex({ serviceRequests }: PageProps) {
    ...
    useEffect(() => {
        ...
        const channel = window.Echo.channel('service.requests')
            .listen('.service.request.created', (e: { serviceRequest: ServiceRequest }) => {
                ...
                toast.info(`Permintaan Baru: ${e.serviceRequest.type.toUpperCase()} di Meja ${e.serviceRequest.reservation?.resto_table?.name || '?'}`);
            });
    ...
    return (
        <>
            <Head title="Service Activity Hub - RestoWeb" />
            ...
            <div className="mx-auto max-w-7xl font-sans text-white pb-12">
                <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
                    <div>
                        <div className="flex items-center gap-2 mb-3">
                            ...
                            <span className="text-[10px] font-black tracking-[0.3em] text-orange-500 uppercase">Hospitality Center</span>
                        </div>
                        <h1 className="font-['Playfair_Display',serif] text-5xl font-black tracking-tighter text-white">
                            Service <span className="text-white/40 italic font-serif">Hub</span>
                        </h1>
                        <p className="mt-4 text-white/40 max-w-lg leading-relaxed font-medium">
                            Kelola semua permintaan langsung dari meja pelanggan secara real-time. Pastikan setiap tamu mendapatkan pelayanan terbaik.
                        </p>
                    </div>
                ...
                <div className="rounded-[3rem] border border-white/5 bg-white/[0.02] shadow-3xl backdrop-blur-3xl overflow-hidden ring-1 ring-white/5">
                    <div className="p-10 border-b border-white/5 bg-white/[0.01]">
                        <h2 className="text-2xl font-black text-white font-['Playfair_Display',serif] tracking-tight">Antrean Permintaan</h2>
                        <p className="text-[10px] font-bold text-white/20 uppercase tracking-[0.2em] mt-1">Live Hospitality Stream</p>
                    </div>
                    ...
                    <div className="divide-y divide-white/5">
                        {activeRequests.length > 0 ? (
                                        ...
                                        <div className="flex gap-8 items-start">
                                            ...
                                            <div className="text-left py-1">
                                                <div className="flex items-center gap-3 mb-2">
                                                    <h3 className="text-xl font-black text-white uppercase tracking-tight">Meja {request.reservation?.resto_table?.name || 'N/A'}</h3>
                                                ...
                                                <p className="text-[11px] font-bold text-white/40 uppercase tracking-widest flex items-center gap-2">
                                                    <AlertCircle size={12} className="text-orange-500/60" />
                                                    Target: {request.reservation?.customer_name || 'Anonymous Guest'}
                                                </p>
                                                {request.notes && (
                            ...
                            <div className="p-32 text-center text-white/10 flex flex-col items-center gap-6">
                                ...
                                <div>
                                    <h3 className="text-xl font-black text-white/40 uppercase tracking-widest">Hening Sempurna</h3>
                                    <p className="text-xs text-white/20 mt-2 max-w-xs mx-auto font-medium">Semua tamu sedang menikmati masakan mereka tanpa gangguan.</p>
                                </div>
                            </div>
                        )}
                    </div>
