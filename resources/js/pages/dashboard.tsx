...
import { dashboard as dashboardRoute, logout } from '@/routes';
...
export default function Dashboard({ currentTeamId, currentTeamSlug, newReservationCount, seatingOverview = [] }: DashboardProps) {
    const [newOrdersCount, setNewOrdersCount] = useState(newReservationCount);
    const [justUpdated, setJustUpdated] = useState(false);
    const reservationsUrl = currentTeamSlug ? reservationsIndex(currentTeamSlug).url : '/';

    useEffect(() => {
        setNewOrdersCount(newReservationCount);
    }, [newReservationCount]);
    ...
    useEffect(() => {
        ...
        const channelName = `team.${currentTeamId}.reservations`;
        const channel = echo.private(channelName);

        channel.listen('.reservation.created', () => {
            setNewOrdersCount((currentCount) => currentCount + 1);
            setJustUpdated(true);
        });

        return () => {
            channel.stopListening('.reservation.created');
            ...
            <div className="flex h-full flex-1 flex-col gap-6 overflow-x-auto bg-slate-50/80 p-6 font-['Inter',sans-serif]">
                ...
                <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
                    ...
                    <div className="flex gap-2">
                        ...
                        <Link href={reservationsUrl}>
                            <Button
                                className={`rounded-xl bg-gradient-to-r text-white shadow-md shadow-amber-900/20 transition-all ${
                                    justUpdated
                                        ? 'scale-[1.03] from-amber-500 to-orange-700'
                                        : 'from-amber-600 to-amber-800 hover:from-amber-500 hover:to-amber-700'
                                }`}
                            >
                                Pesanan Baru
                                {newOrdersCount > 0 && (
                ...
                {seatingOverview.length > 0 && (
                    <Card className="overflow-hidden border-0 bg-white shadow-sm ring-1 ring-slate-900/5">
                        <CardHeader className="border-b border-slate-100 pb-4">
                            <CardTitle className="text-base font-semibold text-slate-900">
                                Seating Chart (Real-time)
                            </CardTitle>
                            <CardDescription>
                                Kursi merah sudah dipesan, kursi hijau masih tersedia.
                            </CardDescription>
                        </CardHeader>
                        <CardContent className="pt-5">
                            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                                {seatingOverview.map((table) => (
                                    <div key={table.id} className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4">
                                        <div className="mb-3 flex items-center justify-between">
                                            <div>
                                                <p className="text-sm font-bold text-slate-900">{table.code}</p>
                                            ...
                                            <span className="rounded-full bg-slate-200 px-2 py-1 text-[10px] font-semibold text-slate-700">
                                                {table.available_seats}/{table.total_seats} tersedia
                                            </span>
                                        </div>
